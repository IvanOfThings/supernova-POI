...
#define MODE_MANUAL  0 // Change images manually via remote
#define MODE_CYCLE   1 // Cycle through images automatically
#define MODE_PERFORM 2 // Use time-coded performance table
uint8_t mode = MODE_MANUAL;

uint32_t CYCLE_TIME = 14; // Time, in seconds, between auto-cycle images

IRrecv irrecv(5); // arg = IR receive pin #
...
void setup() {
  ...
  //FastLED.addLeds<LED_TYPE,DATA_PIN,COLOR_ORDER>(leds, NUM_FASTLEDS).setCorrection(TypicalLEDStrip);
...
void setImage(int i) {
  if(i >= NUM_IMAGES) i = NUM_IMAGES - 1;
  else if(i < 0)      i = 0;
  imageNumber = i;
  imageInit();
}
...
void loop() {
  ...
  // Process any input until it's time to refresh the LEDs:
      ...
      if(!strip.getBrightness()) { // If strip is off...
        ...
        switch(results.value) {
         ...
         case BTN_BRIGHT_DOWN:
          ...
          break;
          case BTN_FASTER:
          // CYCLE_TIME = CYCLE_TIME + 1;  // CYCLE TIME  // Uncomment this line to use button to change cycle time
          // if(lineIntervalIndex < (sizeof(lineTable) / sizeof(lineTable[0]) - 1))  // SCAN SPEED uncomment these 2 lines
          //  lineInterval = pgm_read_word(&lineTable[++lineIntervalIndex]);
          break;
         case BTN_SLOWER:
         // if(CYCLE_TIME==0)    // Uncomment this line to use button to change cycle time
          //  CYCLE_TIME=1;
         // else(CYCLE_TIME = CYCLE_TIME-1); 
         //   if(lineIntervalIndex)  // SCAN SPEED uncomment these 2 lines
          //   lineInterval = pgm_read_word(&lineTable[--lineIntervalIndex]);
          break;
         case BTN_RESTART:
          setImage(0);
         ...
         case BTN_PATTERN_NEXT:
          nextImage();
          break;
         case BTN_AUTOPLAY: // AUTO-PLAY ON/OFF
          if(mode != MODE_CYCLE) { // If not currently in auto-cycle mode...
            mode = MODE_CYCLE;     //  set to auto-cycle,
            setImage(0);           //  starting from first image
          } else {                 // If currently in auto-cycle mode...
            mode = MODE_MANUAL;    //  set to manual (maintain current image)
          }
          break;
         case BTN_7: // START or RESTART PERFORMANCE
          mode = MODE_PERFORM;
          RestartPerformance();
          break;
         case BTN_8: // RESET TO MANUAL MODE & FIRST IMAGE
          mode = MODE_MANUAL;
          setImage(0);
          break;
        }
